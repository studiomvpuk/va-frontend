...
function internalLinks(): { file: string; href: string }[] {
  ...
  const PATTERNS = [
     ...
     *
     *   `${window.location.origin}/accept-invite?token=${t}`
     *
     * This is the shape that got away. The assistants page has always built the
     * invitation link this way, the API has always served the endpoint behind
     * it, and the page in the middle did not exist — so every invitation led to
     * a 404 while this suite reported every link resolving. None of the
     * patterns above can see a path that never appears next to the word `href`.
     *
     * Captures the literal path that follows an interpolated origin, stopping
     * at the query string.
     */
    /\$\{[^}]*(?:origin|SITE_URL|baseUrl)[^}]*\}(\/[^`?#$]*)/g,
  ];

  for (const file of walkSources(root)) {
    // Comments discuss these paths — this suite's own do, at length. Strip them
    // so the scanner reads code rather than the prose explaining the code.
...
/** Served by app/robots.ts and app/sitemap.ts rather than by a page. */
const GENERATED_FILES = ['/sitemap.xml', '/robots.txt'];
  ...
  it('points at a route that exists', () => {
    const broken = links
      .filter(({ href }) => {
        ...
        // Generated by Next from route handlers, not page.tsx files. Real URLs,
        // but they will never appear in ROUTES.
        if (GENERATED_FILES.includes(path)) return false;
